<!--
  Composant de débogage pour afficher les messages SSE.
  Version "Tab" (Onglet), destinée à être
  placée à l'intérieur d'un autre composant.
-->
<script lang="ts">
  import { events } from '$lib/stores/event-store.js';

  // Interface pour définir la structure d'un message de log
  interface LogEntry {
    type: string;
    timestamp: Date;
    content: any;
  ...
  // S'abonne au store 'events'
  $: {
    if ($events) {
      if ($events.type === 'connected') {
        status = 'connected';
