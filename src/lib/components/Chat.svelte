<script lang="ts">
  ...
  // 1. Correction du chemin d'importation
  ...
  function pinMessage(message: ChatMessage) {
    // Votre code ici fonctionne car 'chat' est un store writable
    $chat.pinnedMessage = message;
    
    // Petite astuce Svelte :
    // Puisque nous modifions une propriété *interne* de l'objet,
    // nous devons notifier Svelte que le store a changé.
    chat.set($chat);
  }
    ...
    {#each $chat.messages as message (message.id)}
      <div class="message">
        <div class="message-content">
          ...
          <!-- Utilise message.message (comme défini dans le type) -->
